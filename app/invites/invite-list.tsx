    ...
    <tr className={isSelected ? "bg-stone-50 dark:bg-stone-800/30" : undefined}>
      ...
      <td className="px-3 py-4 text-sm text-stone-500 dark:text-stone-400 font-mono whitespace-nowrap min-w-[150px]">
        {invite.redeemedBy ? (
          ...
          >
            {`${invite.redeemedBy.slice(0, 4)}...${invite.redeemedBy.slice(-4)}`}
            <span
              className={`text-[10px] uppercase font-sans font-bold px-1 rounded transition-opacity ${isCopied ? "bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-300 opacity-100" : "opacity-0 group-hover:opacity-40"}`}
            >
