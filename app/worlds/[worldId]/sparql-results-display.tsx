...
export function SparqlResultsDisplay({ results, loading }: SparqlResultsDisplayProps) {
  if (loading) {
    return (
      <div className="h-full flex flex-col items-center justify-center p-8 text-stone-500">
        <div className="w-8 h-8 mb-4">
          <svg className="animate-spin w-full h-full text-amber-600" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
    ...
    return (
      <div className="p-4 h-full flex items-center justify-center">
        <div className="text-center">
            <span className={`text-4xl font-bold ${results.boolean ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}>
            {results.boolean ? "TRUE" : "FALSE"}
            </span>
            <p className="text-stone-500 mt-2 text-sm uppercase tracking-wider font-semibold">Boolean Result</p>
        </div>
      </div>
    );
  }
      ...
      <div className="overflow-auto h-full">
        <table className="w-full text-left font-mono text-sm">
          <thead className="bg-stone-100 dark:bg-stone-800 sticky top-0 z-10">
            <tr>
              {vars.map((v) => (
                <th key={v} className="px-4 py-2 border-b border-stone-200 dark:border-stone-700 text-stone-700 dark:text-stone-300 font-semibold whitespace-nowrap">
                  ?{v}
                </th>
              ))}
            </tr>
            ...
            {bindings.map((binding, i) => (
              <tr key={i} className="hover:bg-stone-50 dark:hover:bg-stone-900/50 transition-colors">
                {vars.map((v) => {
                   ...
                   return (
                    <td key={v} className="px-4 py-2 text-stone-900 dark:text-stone-100 whitespace-nowrap max-w-xs truncate" title={cell?.value || ""}>
                        {cell ? (
                             cell.type === 'uri' ? (
                                <span className="text-blue-600 dark:text-blue-400 hover:underline cursor-pointer">{cell.value}</span>
                             ) : (
