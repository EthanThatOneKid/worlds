...
import type { CreateToolsOptions, SourceOptions } from "./types";
import { createExecuteSparqlTool } from "./tools/execute-sparql";
import { createSearchFactsTool } from "./tools/search-facts";
import { createGenerateIriTool } from "./tools/generate-iri";
 ...
 */
export function normalizeSources(sources?: SourceOptions[]): SourceOptions[] {
...
export function isUpdateQuery(query: string): boolean {
  ...
  const afterPrologue = normalized
    .slice(prologueMatch?.[0]?.length ?? 0)
    .trim();
...
/**
 * generateUserIri generates an IRI for a user.
 */
export function generateUserIri(accountId: string): string {
  return `https://wazoo.tech/users/${accountId}`;
}

/**
 * generateAssistantIri generates an IRI for an assistant.
 */
export function generateAssistantIri(accountId: string): string {
  return `${generateUserIri(accountId)}#assistant`;
}

/**
 * createTools creates a set of AI SDK tools for interacting with worlds.
 */
export function createTools(options: CreateToolsOptions): {
  ...
  if (options.write) {
    ...
    if (!defaultSource) {
      throw new Error("Write mode requires a default source to be configured");
    }
  }
