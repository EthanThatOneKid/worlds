...
import { getDefaultSource, normalizeSources } from "./ai-utils";
 ...
 */
export function formatDefaultSourceInfo(options: CreateToolsOptions): string {
 ...
 */
export function formatAvailableWorldsInfo(options: CreateToolsOptions): string {
...
export function formatPrompt(options: FormatPromptOptions): string {
  ...
  if (options.date) {
    parts.push(`The time of writing is ${options.date}.`);
  }
