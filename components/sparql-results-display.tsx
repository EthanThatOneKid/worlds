...
export function SparqlResultsDisplay({
  ...
  if (loading) {
    return (
      <div
        className={`flex flex-col items-center justify-center p-${compact ? "4" : "8"} text-stone-500`}
      >
        <Loader2Icon
          className={`${compact ? "w-5 h-5" : "w-8 h-8"} mb-2 animate-spin text-amber-600`}
        />
  ...
  if ("message" in results) {
    ...
    return (
      <div
        className={`${compact ? "p-2" : "p-4"} text-stone-900 dark:text-stone-100`}
      >
  ...
  if ("boolean" in results) {
    return (
      <div
        className={`${compact ? "p-2" : "p-4"} flex items-center ${compact ? "justify-start" : "justify-center h-full"}`}
      >
  ...
  if (hasHead && hasResults) {
    ...
    if (vars.length > 0 || bindings.length > 0) {
      if (bindings.length === 0) {
        return (
          <div
            className={`${compact ? "p-2" : "p-4"} text-stone-500 italic text-sm`}
          >
            No results found.
          </div>
        );
      ...
      return (
        <div className="overflow-auto h-full">
          <table
            className={`w-full text-left font-mono ${compact ? "text-xs" : "text-sm"}`}
          >
